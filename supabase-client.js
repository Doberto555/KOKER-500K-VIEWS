...
// Prefix ki dwe parèt devan non tout moun (sou lis piblik la ak nan kontak VCF/WhatsApp yo)
const NAME_PREFIX = "J1 ";
function withPrefix(name) {
  if (!name) return name;
  return name.trim().startsWith(NAME_PREFIX) ? name.trim() : `${NAME_PREFIX}${name.trim()}`;
}
...
function downloadVCF(contacts, filename = "joker500k-contacts.vcf") {
  ...
  let vcf = "";
  limited.forEach((c) => {
    const prefixedName = withPrefix(c.name);
    vcf += "BEGIN:VCARD\r\n";
    vcf += "VERSION:3.0\r\n";
    vcf += `FN:${prefixedName}\r\n`;
    vcf += `N:${prefixedName};;;;\r\n`;
